...
def read_aloud():
    ...
    if driver:
        try:
            ...
            link_texts = [link.text for link in links if link.text]  # Only include non-empty link texts

            index = 0
            while index < len(link_texts):
                # Read aloud ten links at a time
                for i in range(10):
                    if index < len(link_texts):
                        speak(link_texts[index])
                        index += 1
                
                # Ask the user if they want to continue
                speak("Do you want to continue reading more links?")
                response = listen()
                
                if response not in ["yes", "yeah", "continue"]:
                    speak("Stopping the read aloud.")
                    break
        except NoSuchElementException:
            speak("Could not find any links to read.")
        except WebDriverException as e:
